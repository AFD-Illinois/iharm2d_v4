...
# Run the entropy_wave test problem at 4 resolutions.

source ./utils.sh

# Make directory for mhdmodes1d outputs
ENTROPY_OUTPUT_DIR="$TESTS_OUTPUT_DIR/entropy_wave"
make_output_dir $ENTROPY_OUTPUT_DIR
...
for rec in "${RECONS[@]}"; do
    ...
    RECON_DIR="$ENTROPY_OUTPUT_DIR/${rec}"
    ...
    # plot convergence
    cd $RECON_DIR
    cp $PROB_DIR/convergence_entropy_wave.py ./
    IFS=,  # Set the Internal Field Separator to a comma
    res_comma_separated="${RESOLUTIONS[*]}"
    unset IFS
    python3 convergence_entropy_wave.py --res=$res_comma_separated
done