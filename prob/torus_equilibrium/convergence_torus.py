...
@click.command()
@click.option('-r', '--res', default="32,64,128,256", help="Resolutions", show_default=True)
def plot_convergence(res):
    ...
    ax.loglog([RES[0], RES[-1]], 10**(-2)*np.asarray([RES[0], RES[-1]])**(-2.), color='black', linestyle='dashed', label='$N^{-2}$')
