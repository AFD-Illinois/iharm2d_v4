/*---------------------------------------------------------------------------------
  ...
  -Inverts from conserved to primitive variables based on Mignone & McKinney 2007
...
double err_eqn(double Bsq, double D, double Ep, double QdB, double Qtsq, double Wp, int *eflag);
...
int U_to_P(struct GridGeom *G, struct FluidState *S, int i, int j, int loc)
{
  ...
  S->P[B1][j][i] = S->U[B1][j][i]/gdet;
  S->P[B2][j][i] = S->U[B2][j][i]/gdet;
  S->P[B3][j][i] = S->U[B3][j][i]/gdet;
  ...
  double D = S->U[RHO][j][i]*lapse/gdet;

  double Bcon[NDIM];
  Bcon[0] = 0.;
  Bcon[1] = S->U[B1][j][i]*lapse/gdet;
  Bcon[2] = S->U[B2][j][i]*lapse/gdet;
  Bcon[3] = S->U[B3][j][i]*lapse/gdet;

  double Qcov[NDIM];
  Qcov[0] = (S->U[UU][j][i] - S->U[RHO][j][i])*lapse/gdet;
  Qcov[1] = S->U[U1][j][i]*lapse/gdet;
  Qcov[2] = S->U[U2][j][i]*lapse/gdet;
  Qcov[3] = S->U[U3][j][i]*lapse/gdet;
  ...
  double Wp = Wp_func(G, S, i, j, loc, &eflag);
  if (eflag)
    return eflag;
  ...
  S->P[U1][j][i] = (gamma/(W + Bsq))*(Qtcon[1] + QdB*Bcon[1]/W);
  S->P[U2][j][i] = (gamma/(W + Bsq))*(Qtcon[2] + QdB*Bcon[2]/W);
  S->P[U3][j][i] = (gamma/(W + Bsq))*(Qtcon[3] + QdB*Bcon[3]/W);

#if ELECTRONS
  for (int idx = KEL0; idx < NVAR ; idx++) {
    S->P[idx][j][i] = S->U[idx][j][i]/S->U[RHO][j][i];
  }
  S->P[KTOT][j][i] = S->U[KTOT][j][i]/S->U[RHO][j][i];
#endif // ELECTRONS
...
inline double Wp_func(struct GridGeom *G, struct FluidState *S, int i, int j, int loc, int *eflag)
{
  ...
  //lower_grid(utcon, utcov, G, i, j, loc);
  for (int mu = 0; mu < NDIM; mu++) {
    utcov[mu] = 0.;
    for (int nu = 0; nu < NDIM; nu++) {
